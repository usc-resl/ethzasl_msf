...
void MSF_Core<EKFState_T>::SetPCore(
  ...
  P_core << 0.0166, 0.0122, -0.0015, 0.0211, 0.0074, 0.0000, 0.0012, -0.0012, 0.0001, -0.0000, 0.0000, -0.0000, -0.0003, -0.0002, -0.0000,
            ...
            -0.0000, -0.0000, -0.0000, -0.0000, -0.0000, -0.0000, -0.0000, -0.0000, -0.0000, 0.1000, -0.0000, 0.0000, 0.0000, 0.0000, 0.0000,
            0.0000, 0.0000, -0.0000, 0.0000, 0.0000, 0.0000, 0.0000, -0.0000, -0.0000, -0.0000, 0.1000, -0.0000, 0.0000, 0.0000, 0.0000,
...
void MSF_Core<EKFState_T>::ProcessIMU(
  ...
  // ******
  Vector3 b_w = currentState->template Get<StateDefinition_T::b_w>();
  Vector3 b_a = currentState->template Get<StateDefinition_T::b_a>();
  std::cout << "gyro: " << b_w[0] << " " << b_w[1] << " " << b_w[2];
  std::cout << "  | acc: " << b_a[0] << " " << b_a[1] << " " << b_a[2] << std::endl;



  const EKFState_T& st = *currentState;
  //int bla = StateDefinition_T::p_ic;
  //const Eigen::Matrix<double, 3, 1> zzz = st.template Get<EKFState_T::StateDefinition_T::p_ic>();
  //Eigen::Quaternion<double> q_ic = st.template Get<StateDefinition_T::q_ic>();

  //std::cout << "pos: " << p_ic[0] << " " << p_ic[1] << " " << p_ic[2];
  //std::cout << "  | rot: " << q_ic.x () << " " << q_ic.y () << " " << q_ic.z () << " " << q_ic.w () << std::endl;
  // ******
